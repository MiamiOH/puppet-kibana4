...
[![Puppet Forge](http://img.shields.io/puppetforge/v/lesaux/kibana5.svg)](https://forge.puppetlabs.com/lesaux/kibana5)
[![Build Status](http://img.shields.io/travis/lesaux/puppet-kibana5.svg)](http://travis-ci.org/lesaux/puppet-kibana5)
...
Install and configure kibana5. Should work on any linux OS.

## Module Description

This module assumes you have a working Elasticsearch installation and indices (usually an 'ELK' stack).  kibana5 only works with recent versions of Elasticsearch (1.4.4 and later).  I recommend using the 'elasticsearch' output with `protocol => 'http'` in Logstash (supported since 1.4.0) as the default `protocol => 'node'` uses ES version 1.1.1 and will prevent kibana5 from connecting.
...
Version of kibana5 that gets installed.  Defaults to the latest version available in the `package_repo_version` you select
...
* See the [kibana5 documentation](https://www.elastic.co/guide/en/kibana/current/kibana-server-properties.html) for a full list of kibana server properties.
